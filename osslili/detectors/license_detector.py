...
class LicenseDetector:
    ...
    def _categorize_license(
        self, file_path: Path, detection_method: str, match_type: str = None
    ) -> tuple[str, str]:
        """
        ...
        if (
            ...
            or file_name.endswith(".gemspec")
            or file_name.endswith(".nuspec")
        ):
    ...
    def _is_valid_license_id(self, license_id: str) -> bool:
        ...
        has_valid_indicator = any(
            indicator in license_lower for indicator in valid_license_indicators
        )
    ...
    def _compile_spdx_patterns(self) -> List[re.Pattern]:
        ...
        return [
            ...
            re.compile(
                r"[Ll]icensed\s+under\s+(?:the\s+)?([A-Za-z0-9\-\.\s]+?)(?:\s+[Ll]icense)?(?:[,\n;]|$)",
                re.IGNORECASE,
            ),
        ]
    ...
    def _detect_licenses_in_file_safe(
        self, file_path: Path, single_file_mode: bool = False
    ...
    def _find_license_files(self, directory: Path) -> List[Path]:
        ...
        scanner = SafeFileScanner(max_depth=self.config.max_recursion_depth, follow_symlinks=False)
    ...
    def _detect_licenses_in_file(
        ...
        if file_size > 10 * 1024 * 1024:  # 10MB
            ...
            content = self.input_processor.read_text_file(
                file_path, max_size=file_size if file_size > 0 else 10 * 1024 * 1024
            )
        ...
        for license in licenses:
            if self._is_valid_license_id(license.spdx_id) or (
                ...
                logger.debug(
                    f"Filtered out false positive license: '{license.spdx_id}' from {file_path}"
                )
    ...
    def _contains_license_text(self, content: str) -> bool:
        ...
        indicator_count = sum(
            1 for indicator in self.license_indicators if indicator in content_lower
        )
        ...
        for i, line in enumerate(lines):
            ...
            if license_start == -1:
                if any(
                    ...
                    for marker in ["license", "copyright", "permission is hereby granted"]
                ):
                    license_start = i
                ...
                if name_elem is not None and name_elem.text:
                    ...
                    licenses.append(
                        DetectedLicense(
                            ...
                            name=(
                                license_info.get("name", normalized_id)
                                if license_info
                                else license_name
                            ),
                            confidence=1.0,
            ...
            for license_id in license_ids:
                ...
                licenses.append(
                    DetectedLicense(
                        spdx_id=license_info["licenseId"] if license_info else normalized_id,
                        name=(
                            license_info.get("name", normalized_id) if license_info else license_id
                        ),
                        confidence=1.0,
                        detection_method=DetectionMethod.TAG.value,
                        source_file=str(file_path),
                ...
                if isinstance(license_value, str):
                    ...
                    for license_id in license_ids:
                        ...
                        licenses.append(
                            DetectedLicense(
                                spdx_id=(
                                    license_info["licenseId"] if license_info else normalized_id
                                ),
                                name=(
                                    license_info.get("name", normalized_id)
                                    if license_info
    ...
    def _detect_license_keywords(self, content: str, file_path: Path) -> List[DetectedLicense]:
        ...
        for spdx_id, variations in base_license_mapping.items():
                ...
                if variation.lower() in content.lower():
                    ...
                    if match:
                        ...
                        if match.start() > 0:
                            ...
                            line_start = (
                                any(line_prefix.startswith(marker) for marker in comment_markers)
                                or any(
                                    keyword in line_prefix.lower() for keyword in license_keywords
                                )
                                or line_prefix == ""
                            )
                        else:
                        ...
                        if has_context and line_start:
                            ...
                            final_spdx_id = handle_version_suffix(
                                spdx_id, content[match.start() : match.end() + 20]
                            )
                            ...
                            if final_spdx_id == "GPL":
                                ...
                                if (
                                    "3" in context_text
                                    or "v3" in context_text
                                    or "version 3" in context_text.lower()
                                ):
                                    final_spdx_id = "GPL-3.0"
                                elif (
                                    "2" in context_text
                                    or "v2" in context_text
                                    or "version 2" in context_text.lower()
                                ):
            ...
            if spdx_id not in found_licenses and spdx_id in [
                "MIT",
                "Apache-2.0",
                "GPL-2.0",
                "GPL-3.0",
            ...
            if license_file_path.exists():
                try:
                    ...
                    if license_content:
                        ...
                        detected_licenses = self._detect_from_full_text(
                            license_content, license_file_path
                        )
                ...
                logger.debug(
                    f"License file {license_file_path} referenced in pyproject.toml does not exist"
                )
    ...
    def _normalize_cc_license(self, license_text: str) -> str:
        ...
        # Handle CC0 first
        if "CC0" in license_text.upper() or (
            "CC" in license_text.upper() and "ZERO" in license_text.upper()
        ):
            return "CC0-1.0"
        ...
        cc_match = re.search(
            r"CC[- ]?(BY|ZERO)?[- ]?(SA|NC|ND)?[- ]?(\d+\.\d+)?", license_text.upper()
        )
        if cc_match:
            parts = ["CC"]
    ...
    def _parse_license_expression(self, expression: str) -> List[str]:
        ...
        if "," in expression and not any(
            op in expression.upper() for op in [" OR ", " AND ", " WITH "]
        ):
    ...
    def _tier0_exact_hash(self, text: str, file_path: Path) -> Optional[DetectedLicense]:
        ...
        if license_id:
            ...
            category, match_type = self._categorize_license(file_path, DetectionMethod.HASH.value)
        ...
        if "Apache-2.0" in license_ids and "Pixar" in license_ids:
            ...
            for lid, score in close_matches:
                if lid == "Apache-2.0":
                    ...
                    best_score = score
                    logger.debug(
                        f"Preferring Apache-2.0 over Pixar (Dice-Sørensen scores within 1%)"
                    )
                    break
    ...
    def _adjust_regex_confidence(
        self, raw_score: float, category: str, match_type: str, match_count: int
    ) -> float:
        """
